...
class SubMenuController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $pageTitle = trans('messages.list_form_title',['form' => trans('messages.sub_menu_category')] );
        $auth_user = authSession();
        $assets = ['datatable'];
        return view('submenu.index', compact('pageTitle','auth_user','assets','filter'));
    }

    public function index_data(DataTables $datatable,Request $request)
    {
        $query = SubMenu::with(['translations', 'menuCategory.translations']);
        ...
        if (auth()->user()->hasAnyRole(['admin'])) {
            ...
            ->editColumn('name', function($query) use($primary_locale){
                ...
                if (auth()->user()->can('sub menu category edit')) {
                    $link = '<a class="btn-link btn-link-hover" href='.route('sub-menu.create', ['id' => $query->id]).'>'.$name.'</a>';
                } else {
                    $link = $name;
                }
                return $link;
            ...
            ->addColumn('action', function ($data) {
                return view('submenu.action', compact('data'))->render();
            })
    ...
    {
        if (!auth()->user()->can('sub menu category add')) {
        ...
        $categorydata = SubMenu::find($id);

        $menuCategories = MenuCategory::where('status', 1)
                            ->orderBy('created_at', 'desc')
                            ->get();

        $pageTitle = trans('messages.update_form_title',['form'=>trans('messages.sub_menu_category')]);

        if($categorydata == null){
            $pageTitle = trans('messages.add_button_form',['form' => trans('messages.sub_menu_category')]);
            $categorydata = new SubMenu;
        }

        return view('submenu.create', compact('pageTitle' ,'categorydata' ,'auth_user','language_array', 'menuCategories' ));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->is('api/*') && is_null($request->id) && !$request->hasFile('category_image')) {
            return redirect()->route('sub-menu.create')
                ->withErrors(__('validation.required', ['attribute' => 'attachments']))
                ->withInput();
        }
        $result = SubMenu::updateOrCreate(['id' => $data['id']], $data);
        if ($request->is('api/*')) {
        ...
        if ($request->hasFile('category_image')) {
            ...
            return redirect()->route('sub-menu.create', ['id' => $result->id])
        ...
        return redirect(route('sub-menu.index'))->withSuccess($message);
    }
    ...
    public function destroy(string $id)
    {
        ...
        $msg= __('messages.msg_fail_to_delete',['name' => __('messages.sub_menu_category')] );
        ...
        if($category!='') {
            ...
            $category->delete();
            $msg= __('messages.msg_deleted',['name' => __('messages.sub_menu_category')] );
        }
        if(request()->is('api/*')) {
    ...
    public function action(Request $request){
        $id = $request->id;
        $category  = SubMenu::withTrashed()->where('id',$id)->first();
        $msg = __('messages.t_found_entry',['name' => __('messages.sub_menu_category')] );
        if($request->type == 'restore') {
            $category->restore();
            $msg = __('messages.msg_restored',['name' => __('messages.sub_menu_category')] );
        }
        if($request->type === 'forcedelete'){
            $category->forceDelete();
            $msg = __('messages.msg_forcedelete',['name' => __('messages.sub_menu_category')] );
        }
        if(request()->is('api/*')){
            return comman_message_response($msg);
		}
    ...
    public function check_in_trash(Request $request)
    {
        ...
        switch($type){
            case 'category':
                $InTrash = SubMenu::withTrashed()->whereIn('id', $ids)->whereNotNull('deleted_at')->get();
            break;
            case 'subcategory':
