...
use App\Models\PackageProductMapping;
use Yajra\DataTables\DataTables;
use App\Models\BookingProductPackageMapping;
...
class ProductPackageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $assets = ['datatable'];
        return view('productpackage.index', compact('pageTitle','auth_user','assets','filter'));
    }
        ...
        ->editColumn('status' , function ($query){
            return '<div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                <div class="custom-switch-inner">
                    <input type="checkbox" class="custom-control-input  change_status" data-type="productpackage_status" '.($query->status ? "checked" : "").'  value="'.$query->id.'" id="'.$query->id.'" data-id="'.$query->id.'">
            ...
            ->editColumn('name', function($query) use($primary_locale){   
                ...
                if (auth()->user()->can('product list')) {
                    $link ='<a class="btn-link btn-link-hover"  href='.route('productpackage.service',$query->id).'>'.$name.'</a>';
                } else {
                    $link = $name; 
                }
                return $link;
            ...
            ->editColumn('provider_id' , function ($query){
                return view('productpackage.service', compact('query'));
            })
            ...
            ->addColumn('action', function ($servicepackage) {
                return view('productpackage.action', compact('servicepackage'))->render();
            })
            ->addIndexColumn()
    ...
    public function bulk_action(Request $request)
    {
        ...
        switch ($actionType) {
            case 'change-status':
                $branches = ProductPackage::whereIn('id', $ids)->update(['status' => $request->status]);
                $message = 'Bulk Product Status Updated';
                break;

            case 'delete':
                ProductPackage::whereIn('id', $ids)->delete();
    ...
    {
        if (!auth()->user()->can('productpackage add')) {
        ...
        $services = [];
        $selectedServiceId = [];
        $servicepackage = ProductPackage::find($id);   
        ...
        if($servicepackage !== null){
            ...
            if (is_array($serviceIds)) {
                    $services = Product::whereIn('id', $serviceIds)->get();
                    $selectedServiceId = $serviceIds;
                }
            }
        if ($servicepackage == null) {
            $pageTitle = trans('messages.add_button_form', ['form' => trans('messages.package')]);
            $servicepackage = new ProductPackage;
        }else{
            if ($servicepackage->provider_id !== auth()->user()->id && !auth()->user()->hasRole(['admin', 'demo_admin'])) {
                return redirect(route('productpackage.index'))->withErrors(trans('messages.demo_permission_denied'));
            }
        }

        return view('productpackage.create', compact('pageTitle', 'servicepackage', 'auth_user','services','selectedServiceId','language_array'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $result = ProductPackage::updateOrCreate(['id' => $data['id']], $service_package);
        if ($request->is('api/*')) {
        ...
        if ($request->is('api/*')) {
            ...
            if ($request->hasFile('package_attachment')) {
                ...
                return redirect()->route('productpackage.create', ['id' => $result->id])
        ...
        return redirect(route('productpackage.index'))->withSuccess($message);
    }
    ...
    public function destroy($id)
    {
        ...
        $service_package = ProductPackage::find($id);
    ...
    public function action(Request $request){
        $id = $request->id;
        $servicepackage = ProductPackage::where('id',$id)->first();
        $msg = __('messages.not_found_entry',['name' => __('messages.product_package')] );
        if($request->type === 'forcedelete'){
            ...
            $msg = __('messages.msg_forcedelete',['name' => __('messages.product_package')] );
        }
