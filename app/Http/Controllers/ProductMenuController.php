...
class ProductMenuController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $pageTitle = trans('messages.list_form_title',['form' => trans('messages.product_menu_category')] );
        $auth_user = authSession();
        $assets = ['datatable'];
        return view('product-menu.index', compact('pageTitle','auth_user','assets','filter'));
    }
    ...
    {
        if (!auth()->user()->can('product menu category add')) {
        ...
        $categorydata = ProductMenuCategory::find($id);

        $pageTitle = trans('messages.update_form_title',['form'=>trans('messages.product_menu_category')]);

        if($categorydata == null){
            $pageTitle = trans('messages.add_button_form',['form' => trans('messages.product_menu_category')]);
            $categorydata = new ProductMenuCategory;
        }

        return view('product-menu.create', compact('pageTitle' ,'categorydata' ,'auth_user','language_array' ));
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->is('api/*') && is_null($request->id) && !$request->hasFile('category_image')) {
            return redirect()->route('product-menu.create')
                ->withErrors(__('validation.required', ['attribute' => 'attachments']))
                ->withInput();
        }
        $result = ProductMenuCategory::updateOrCreate(['id' => $data['id']], $data);
        if ($request->is('api/*')) {
        ...
        if ($request->hasFile('category_image')) {
            ...
            return redirect()->route('product-menu.create', ['id' => $result->id])
        ...
        return redirect(route('product-menu.index'))->withSuccess($message);
    }
    ...
    public function destroy(string $id)
    {
        ...
        $category = ProductMenuCategory::find($id);

        $msg= __('messages.msg_fail_to_delete',['name' => __('messages.product_menu_category')] );
        ...
        if($category!='') {
            ...
            $category->delete();
            $msg= __('messages.msg_deleted',['name' => __('messages.product_menu_category')] );
        }
        if(request()->is('api/*')) {
    ...
    public function action(Request $request){
        $id = $request->id;
        $category  = ProductMenuCategory::withTrashed()->where('id',$id)->first();
        $msg = __('messages.t_found_entry',['name' => __('messages.product_menu_category')] );
        if($request->type == 'restore') {
            $category->restore();
            $msg = __('messages.msg_restored',['name' => __('messages.product_menu_category')] );
        }
        if($request->type === 'forcedelete'){
            $category->forceDelete();
            $msg = __('messages.msg_forcedelete',['name' => __('messages.product_menu_category')] );
        }
        if(request()->is('api/*')){
            return comman_message_response($msg);
		}
    ...
    public function check_in_trash(Request $request)
    {
        ...
        switch($type){
            case 'category':
                $InTrash = ProductMenuCategory::withTrashed()->whereIn('id', $ids)->whereNotNull('deleted_at')->get();
            break;
            case 'subcategory':
