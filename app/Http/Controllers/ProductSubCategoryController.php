...
class ProductSubCategoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $pageTitle = trans('messages.list_form_title',['form' => trans('messages.product_subcategory')] );
        $auth_user = authSession();
        $assets = ['datatable'];
        return view('product-subcategory.index',compact('pageTitle','auth_user','assets','filter'));
    }

    public function index_data(DataTables $datatable,Request $request)
    {
        $query = ProductSubCategory::query()->with(['translations', 'category.translations']);
        if (!$request->order) { 
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('name', function($row) use ($primary_locale){                
                ...
                if (auth()->user()->can('product subcategory edit')) {
                    return '<a class="btn-link btn-link-hover" href="' . route('product-subcategory.create', ['id' => $row->id]) . '">' . $name . '</a>';
                }
            ...
            ->editColumn('is_featured' , function ($query){
                ...
                return '<div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                    <div class="custom-switch-inner">
                        <input type="checkbox" class="custom-control-input change_status" data-type="product_subcategory_featured" data-name="is_featured" '.($query->is_featured ? "checked" : "").'  '.  $disabled.' value="'.$query->id.'" id="f'.$query->id.'" data-id="'.$query->id.'">
                        <label class="custom-control-label" for="f'.$query->id.'" data-on-label="'.__("messages.yes").'" data-off-label="'.__("messages.no").'"></label>
                    </div>
                </div>';
            })
            ->addColumn('action', function ($data) {
                return view('product-subcategory.action', compact('data'));
            })
            ->editColumn('status' , function ($query){
                $disabled = $query->trashed() ? 'disabled': '';
                return '<div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                    <div class="custom-switch-inner">
                        <input type="checkbox" class="custom-control-input  change_status" data-type="product_subcategory_status" '.($query->status ? "checked" : "").'  '.$disabled.' value="'.$query->id.'" id="'.$query->id.'" data-id="'.$query->id.'">
    ...
    public function bulk_action(Request $request)
    {
        ...
        switch ($actionType) {
            case 'change-status':
                $branches = ProductSubCategory::whereIn('id', $ids)->update(['status' => $request->status]);
                $message = 'Bulk Sub Category Status Updated';
                break;
            
            case 'change-featured':
                $branches = ProductSubCategory::whereIn('id', $ids)->update(['is_featured' => $request->is_featured]);
            ...
            case 'restore':
                ProductSubCategory::whereIn('id', $ids)->restore();
                $message = 'Bulk Sub Category Restored';
                break;
                
            case 'permanently-delete':
                ProductSubCategory::whereIn('id', $ids)->forceDelete();
    ...
    {
        if (!auth()->user()->can('product subcategory add')) {
        ...
        $pageTitle = trans('messages.update_form_title',['form'=>trans('messages.product_subcategory')]);
        
        if($subcategory == null){
            $pageTitle = trans('messages.add_button_form',['form' => trans('messages.product_subcategory')]);
            $subcategory = new ProductSubCategory;
        }
        
        return view('product-subcategory.create', compact('pageTitle' ,'subcategory' ,'auth_user','language_array' ));
    }
    ...
    public function store(SubCategoryRequest $request)
    {
        ...
        $result = ProductSubCategory::updateOrCreate(['id' => $data['id'] ],$data);
        if ($request->is('api/*')) {
        ...
        $result->saveTranslations($data, $translatableAttributes, $language_option, $primary_locale);
         if ($request->hasFile('subcategory_image')) {
             ...
             return redirect()->route('product-subcategory.create', ['id' => $result->id])
             ->withErrors(['subcategory_image' => 'The attachments field is required.'])
             ->withInput();
         }

        $message = trans('messages.update_form',['form' => trans('messages.product_subcategory')]);
        if($result->wasRecentlyCreated){
            $message = trans('messages.save_form',['form' => trans('messages.product_subcategory')]);
        }
        if($request->is('api/*')) {
            return comman_message_response($message);
		}
        return redirect(route('product-subcategory.index'))->withSuccess($message);    
    }
    ...
    public function destroy($id)
    {
        ...
        $subcategory = ProductSubCategory::find($id);
        $msg= __('messages.msg_fail_to_delete',['name' => __('messages.product_subcategory')] );
        
        if($subcategory!='') { 
            $subcategory->delete();
            $msg= __('messages.msg_deleted',['name' => __('messages.product_subcategory')] );
        }
        if(request()->is('api/*')) {
    ...
    public function action(Request $request){
        ...
        $subcategory  = ProductSubCategory::withTrashed()->where('id',$id)->first();
        $msg = __('messages.not_found_entry',['name' => __('messages.product_subcategory')] );
        if($request->type == 'restore') {
            $subcategory->restore();
            $msg = __('messages.msg_restored',['name' => __('messages.product_subcategory')] );
        }
        if($request->type === 'forcedelete'){
            $subcategory->forceDelete();
            $msg = __('messages.msg_forcedelete',['name' => __('messages.product_subcategory')] );
        }
        if(request()->is('api/*')){
            return comman_message_response($msg);
		}
