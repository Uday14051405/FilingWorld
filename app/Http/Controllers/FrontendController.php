...
class FrontendController extends Controller
{
    ...
    public function index(Request $request){
        ...
        $sectionData = [];
        $sectionKeys = ['section_1', 'section_2', 'section_3', 'section_4', 'section_5', 'section_6', 'section_7','section_8', 'section_9','section_10','section_11','section_18','section_19'];
        ...
        return view('landing-page.index',compact('sectionData','postjobservice','auth_user_id','favourite','totalRating','status','auth_user_type'));
    }

    public function getMenuCategories()
    {
        $record = FrontendSetting::where('key', 'section_14')
            ->value('value'); // this directly fetches the value column

        if ($record) {
            $json = json_decode($record, true);

            $serviceIds = $json['service_id'] ?? [];
        } else {
            $serviceIds = [];
        }

        $menuCategories = MenuCategory::where('status', 1)
            ->whereIn('id', $serviceIds)
            ->orderBy('order_by')
            ->with(['submenus' => function ($query) {
                $query->where('status', 1)->orderBy('order_by');
            }])
            ->get();
    ...
    {
        $record = FrontendSetting::where('key', 'section_12')
            ->value('value'); // this directly fetches the value column
    ...
    {
        $record = FrontendSetting::where('key', 'section_15')
            ->value('value'); // this directly fetches the value column
        ...
        $menuCategories = ProductCategory::where('status', 1)
            ->whereIn('id', $serviceIds)
            ->with(['subCategories' => function ($query) {
                $query->where('status', 1);
            }])->get();
    ...
    {
        $record = FrontendSetting::where('key', 'section_13')
            ->value('value'); 
        ...
        $menuCategories = SubCategory::where('status', 1)
            ->whereIn('id', $serviceIds)
    ...
    public function getProductSubcategoryMenu()
    {
        $record = FrontendSetting::where('key', 'section_16')
            ->value('value'); 
        ...
        $menuCategories = ProductSubCategory::where('status', 1)
            ->whereIn('id', $serviceIds)
            ->with(['services' => function ($query) {
                $query->where('status', 1);
            }])
    ...
    public function getProductMenuCategories()
    {
        $record = FrontendSetting::where('key', 'section_17')
            ->value('value'); // this directly fetches the value column
        ...
        $menuCategories = ProductMenuCategory::where('status', 1)
            ->whereIn('id', $serviceIds)
            ->orderBy('order_by')
            ->with(['submenus' => function ($query) {
                $query->where('status', 1)->orderBy('order_by');
    ...
    public function getProductCategoryCategories()
    {
        $menuCategories = ProductCategory::where('status', 1)
            ->with(['subCategories' => function ($query) {
                $query->where('status', 1);
    ...
    public function catgeoryList(Request $request){
        ...
        if($request->query('submenu_id')){
            $submenuId = $request->query('submenu_id');
        }else{
            $submenuId = null;
        }

        return view('landing-page.category', compact('submenuId'));
    }

    public function categoryListFilter(Request $request) {
        $headerSection = FrontendSetting::where('key', 'heder-menu-setting')->first();
        $sectionData = $headerSection ? json_decode($headerSection->value, true) : null;
        ...
        $submenuId = $request->input('submenu_id'); // Fetch from POST data instead of query
        $category_type = $request->input('category_type');

        return view('landing-page.category', compact('submenuId'));
    }

    public function productCategoryListFilter(Request $request) {
        $headerSection = FrontendSetting::where('key', 'heder-menu-setting')->first();
        $sectionData = $headerSection ? json_decode($headerSection->value, true) : null;
        ...
        return view('landing-page.product-category', compact('submenuId'));
    }
    ...
    public function productSubCatgeoryList(Request $request){
        $category_id = $request->category_id;
        return view('landing-page.product-sub-category', compact('category_id'));
    }
    ...
    public function serviceList(Request $request){
        ...
        return view('landing-page.service', compact('id','type','latitude','longitude','section'));
    }
    ...
    public function productList(Request $request){
        ...
        return view('landing-page.product', compact('id','type','latitude','longitude','section'));
    }
    ...
    public function productCategoryDetail(Request $request){
        ...
        $category->name = $this->getTranslation($category->translations, $primary_locale, 'name', $category->name) ?? $category->name;
        $category->description = $this->getTranslation($category->translations, $primary_locale, 'description', $category->description) ?? $category->description;
        $sub_category = ProductSubCategory::with('translations')
        ->where('category_id', $category_id)
        ->where('status', 1)
        ->get()
        ->map(function ($subCategory) use ($primary_locale) {
        ...
        $serviceController = app(ProductController::class);
        $apiRequest = new Request(['category_id' => $category_id, 'per_page' => 'all']);
        // Set the language code header
        ...
        return view('landing-page.product-category-detail', compact('category','sub_category', 'service','primary_locale', 'auth_user_type'));
    }
    ...
    public function productDetail(Request $request){
        ...
        $knownLanguageArray = json_decode($serviceData['provider']['known_languages'], true);
        $subtotal = $serviceData['service_detail']['discount'] != 0 ?($serviceData['service_detail']['price'])-($serviceData['service_detail']['price']*$serviceData['service_detail']['discount']/100) : $subtotal = $serviceData['service_detail']['price'];
        $total_ratings = BookingRating::where('service_id',$serviceData['service_detail']['id'])->where('type', 'product')->get();
        $type = 'product';
        return view('landing-page.ProductDetail', compact('serviceData','favouriteService','date_time','completed_services','knownLanguageArray','subtotal','total_ratings','favouriteServiceData','userId','type'));
    }
    ...
    public function bookProductView(Request $request){

        $service_id=$request->id;
        $service = Product::where('id',$service_id)->with('providers','category','serviceRating','serviceAddon')->first();
        $primary_locale = session()->get('locale', 'en');
        $service->name = $this->getTranslation($service->translations, $primary_locale, 'name', $service->name) ?? $service->name;
        $service->service_image = getSingleMedia($service,'service_attachment', null);
        $service->category_name = $this->getTranslation(optional($service->category)->translations, $primary_locale, 'name', optional($service->category)->name ?? null) ?? optional($service->category)->name;
        ...
        if($request->package_id){
            $service = ProductPackage::where('id', $request->package_id)->first();
        ...
        if($request->addons){
            $addon_id = explode(',', $request->addons);
            $serviceaddons = ProductAddon::whereIn('id', $addon_id)->get();
        ...
        $wallet = $user ? $user->wallet : null;
        $wallet_amount = $wallet ? $wallet->amount : 0;
        return view('landing-page.BookProduct',compact('service','coupons','taxes','user_id','availableserviceslot','serviceaddon','googlemapkey','wallet_amount'));
    }
    ...
    public function bookingDetail(Request $request){
        ...
        $uploadedDocuments = DocumentUpload::where('order_id', $bookingData['booking_detail']['id'])->get();

        return view('landing-page.BookingDetail',compact('booking','wallet_amount','date_time','bookingData','advancepaymentamount', 'uploadedDocuments'));
    }
    ...
    public function productPackageList(Request $request){
        ...
        $service = $serviceController->getServiceDetail($apiRequest);
        $serviceData = json_decode($service->content(), true);
        return view('landing-page.ProductPackages',compact('service','serviceData'));
    }
    ...
    public function categoryDatatable(Datatables $datatable, Request $request){
        ...
        // Filter by submenuId if provided
        if ($request->has('submenu_id')) {
            $query->where('submenu_category', $request->submenu_id);
        }
    ...
    public function productCategoryDatatable(Datatables $datatable, Request $request){
        ...
        $datatable = $datatable->eloquent($query)
            ->editColumn('name', function ($data) use($primary_locale){
                $data->name = $this->getTranslation($data->translations, $primary_locale, 'name', $data->name) ?? $data->name;
                $data->description = $this->getTranslation($data->translations, $primary_locale, 'description', $data->description) ?? $data->description;
                return view('product-category.datatable-card',compact('data','primary_locale'));
            })
            ->order(function ($query) {
                $query->orderBy('id', 'desc');
            });
    ...
    public function productSubCategoryDatatable(Datatables $datatable, Request $request){
        $query = ProductSubCategory::query();
        $query=$query->where('status',1);
        $query = $query->where('category_id', $request->category_id);
        $datatable = $datatable->eloquent($query)
            ->editColumn('name', function ($data) {
                return view('product-subcategory.datatable-card',compact('data'));
            })
            ->order(function ($query) {
                $query->orderBy('id', 'desc');
            });
    ...
    public function serviceDatatable(Datatables $datatable, Request $request){
        if($request->section){
            ...
            $query = Service::where('service_type','service')->where('status',1)->orderByRaw('FIELD(id, '.implode(',', $serviceIds).') DESC');
        }else{
        ...
        ->editColumn('name', function ($data) use($primary_locale){
            $data->name = $this->getTranslation($data->translations, $primary_locale, 'name', $data->name) ?? $data->name;
            $totalReviews = $data->id ? BookingRating::where('service_id', $data->id)->where('type', '!=', 'product')->count() : 0;
    ...
    public function productDatatable(Datatables $datatable, Request $request){
        if($request->section){
            ...
            $query = Product::where('service_type','product')->where('status',1)->orderByRaw('FIELD(id, '.implode(',', $serviceIds).') DESC');

            $cat_query = ProductCategory::where('submenu_category', 16)->pluck('id');

            $cat_ids = $cat_query->toArray();
            if($request->section == 'section_10'){
                $query = $query->whereNotIn('category_id', $cat_ids);
            }elseif($request->section == 'section_11'){
                $query = $query->whereIn('category_id', $cat_ids);
            }
        }else{
            $query = Product::where('service_type','product')->where('status',1);
        }
        ...
        if ($request->has('latitude') && !empty($request->latitude) && $request->has('longitude') && !empty($request->longitude)) {
            ...
            $query->with('providerProductAddress')->whereIn('id',$service_in_location);
        }
        ...
        ->editColumn('name', function ($data) use($primary_locale){
            ...
            return view('product.datatable-card', compact('data', 'totalReviews','totalRating', 'favouriteService'));
        })
        ->order(function ($query) {
            $query->orderBy('id', 'desc');
        });
    ...
    public function blogDatatable(Datatables $datatable, Request $request){
        ...
        if ($request->has('sec')) {
            ...
            $frontendSetting = FrontendSetting::where('key', $sec)->first();

            if ($frontendSetting) {
                $valueData = json_decode($frontendSetting->value, true);

                if (isset($valueData['service_id']) && is_array($valueData['service_id'])) {
                    $serviceIds = $valueData['service_id'];
    ...
    public function bookingDatatable(Datatables $datatable, Request $request)
    {
        ...
        if (isset($filter['search'])) {
            $query->where(function ($q) use ($filter, $primary_locale) {
                ...
                // For product type
                $q->orWhere(function ($q) use ($filter, $primary_locale) {
                    $q->where('type', 'product')
                        ->whereHas('product', function ($q) use ($filter, $primary_locale) {
                            if ($primary_locale !== 'en') {
                                $q->where(function ($q) use ($filter, $primary_locale) {
                                    $q->whereHas('translations', function ($q) use ($filter, $primary_locale) {
                                        $q->where('locale', $primary_locale)
        ...
        if (isset($filter['booking_date_range'])) {
            [$startDate, $endDate] = explode(' to ', $filter['booking_date_range']);
        ...
        $datatable = $datatable->eloquent($query)
            ->editColumn('name', function ($data) use ($primary_locale) {
                ...
                // Get service or product name, image, rating
                if ($data->type === 'service' && $data->service) {
                    $service = $data->service;
                    $servicename = $this->getTranslation(optional($service->translations), $primary_locale, 'name', $service->name) ?? $service->name;
                    $serviceimage = getSingleMedia($service, 'service_attachment', null);
                    $total_rating = (float) number_format(max(optional($service->serviceRating)->avg('rating'), 0), 2);
                } elseif ($data->type === 'product' && $data->product) {
                    $product = $data->product;
                    $servicename = $this->getTranslation(optional($product->translations), $primary_locale, 'name', $product->name) ?? $product->name;
                    $serviceimage = getSingleMedia($product, 'service_attachment', null);
                    $total_rating = (float) number_format(max(optional($product->productRating)->avg('rating'), 0), 2);
                }

                // Booking package name (if exists)
                $servicepackagename = null;
                if (isset($data->bookingPackage)) {
                    $servicepackagename = $this->getTranslation(
                        optional(optional($data->bookingPackage)->package)->translations,
                        $primary_locale,
                        'name',
                        optional(optional($data->bookingPackage)->package)->name
                    ) ?? optional($data->bookingPackage)->name;
                }

                // Refund logic
                $advancepaid = $data->status === 'cancelled'
                    ? ($data->advance_paid_amount == null ? 0 : (double) $data->advance_paid_amount)
                    : 0;
                ...
                return view('booking.datatable-card', compact(
                    'servicepackagename',
                    'data',
                    'servicename',
                    'payment',
    ...
    public function ratingDatatable(Datatables $datatable, Request $request){
        ...
        elseif($request->type == 'product'){
            $query = $query->where('service_id', $id)->where('type','product');
