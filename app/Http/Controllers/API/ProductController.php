...
class ProductController extends Controller
{
    public function getServiceList(Request $request){
        ...
        if($request->section){
            ...
            $service = Product::where('service_type','product')->with(['providers','category','serviceRating','translations'])->orderBy('created_at','desc');
        }

        // $service = Product::where('service_type','product')->with(['providers','category','serviceRating','translations'])->orderBy('created_at','desc');
        $category = ProductCategory::onlyTrashed()->get();
    ...
    public function getServiceDetail(Request $request){
        ...
        if(auth()->user() !== null){
            if(auth()->user()->hasRole('admin')){
                $service = Product::where('service_type','product')->withTrashed()->with('providers','category','serviceRating','serviceAddon','translations')->findorfail($id);
            }
            else{
                $service = Product::where('service_type','product')->with('providers','category','serviceRating','serviceAddon','translations')->findorfail($id);
            }
        }else{
            $service = Product::where('service_type','product')->where('status',1)->with('providers','category','serviceRating','serviceAddon','translations')->find($id);
        }
        ...
        $related = $service->where('service_type','product')->where('category_id',$service->category_id);
         if(default_earning_type() === 'subscription'){
        ...
        $servicefaq =  ProductFaq::where('service_id',$id)->where('status',1)->get();
        $serviceAddon = ProductAddon::where('service_id',$id)->where('status',1)->get();
    ...
    public function getUserFavouriteService(Request $request)
    {
        ...
        $favourite = UserFavouriteProduct::where('user_id',$user->id);
    ...
    public function saveServiceCoupon(Request $request){
        ...
        if($result){
            ...
            CouponProductMapping::Create($service_data);
        }
