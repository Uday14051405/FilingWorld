...
class DashboardController extends Controller
{
    ...
    public function dashboardDetail(Request $request){
        ...
        $subcategory_section = FrontendSetting::getValueByKey('section_18');
        $subcategory= SubCategoryResource::collection( SubCategory::whereIN( 'id' ,$subcategory_section->subcategory_id )->paginate(8));

        // $category_section = FrontendSetting::getValueByKey('section_2');
        // $category= CategoryResource::collection( Category::whereIN( 'id' ,$category_section->category_id )->orderBy('order_by','asc')->paginate(8));
        ...
        $product = Product::where('status',1)->where('service_type','product');
        $product = $product->whereHas('providers', function ($a) use ($request) {
            $a->where('status', 1);
        });
        ...
        $product = $product->orderBy('id','desc')->paginate($per_page);
        ...
        $product = ServiceResource::collection($product);
        ...
        if ($request->has('latitude') && !empty($request->latitude) && $request->has('longitude') && !empty($request->longitude)) {
            ...
            $locations_product = Product::locationService($request->latitude,$request->longitude,$get_distance,$get_unit);
            $product_in_location = ProviderServiceAddressMapping::whereIn('provider_address_id',$locations_product)->get()->pluck('service_id');
            $product = Product::with('providerServiceAddress')->whereIn('id',$product_in_location)->orwhere('visit_type','online')->get();
            $product = ServiceResource::collection($product);
        }
        ...
        $featured_product_section = FrontendSetting::getValueByKey('section_11');
        $featured_product= Product::whereIN( 'id' ,$featured_product_section->service_id );
        $featured_product = $featured_product->whereHas('providers', function ($a) use ($request) {
            $a->where('status', 1);
        });

        $featured_hot_selling_section = FrontendSetting::getValueByKey('section_10');
        $featured_hot_selling= Product::whereIN( 'id' ,$featured_hot_selling_section->service_id );
        $featured_hot_selling = $featured_hot_selling->whereHas('providers', function ($a) use ($request) {
            $a->where('status', 1);
        });
        ...
        $featured_product = ServiceResource::collection($featured_product->orderBy('id','desc')->paginate($per_page));
        $featured_hot_selling = ServiceResource::collection($featured_hot_selling->orderBy('id','desc')->paginate($per_page));
        ...
        if ($request->has('latitude') && !empty($request->latitude) && $request->has('longitude') && !empty($request->longitude)) {
            ...
            $locations_product = Product::locationService($request->latitude,$request->longitude,$get_distance,$get_unit);
            $product_in_location = ProviderServiceAddressMapping::whereIn('provider_address_id',$locations)->get()->pluck('service_id');
            $featured_product = Product::with('providerServiceAddress')->whereIn('id',$product_in_location)->where('is_featured',1) ->get();
            $featured_product = ServiceResource::collection($featured_product);
            
            $locations_product = Product::locationService($request->latitude,$request->longitude,$get_distance,$get_unit);
            $product_in_location = ProviderServiceAddressMapping::whereIn('provider_address_id',$locations)->get()->pluck('service_id');
            $featured_hot_selling = Product::with('providerServiceAddress')->whereIn('id',$product_in_location)->where('is_featured',1) ->get();
            $featured_hot_selling = ServiceResource::collection($featured_hot_selling);
        }
        ...
        $blog_section = FrontendSetting::getValueByKey('section_19');
        $blogs = BlogResource::collection(Blog::whereIN( 'id' ,$blog_section->service_id )->paginate(4));
        ...
        //    'category'       => $category,
           ...
           'hot_selling'        => $featured_hot_selling,
           'product_accessories' => $featured_product,
           'provider'       => $provider,
    ...
    public function providerDashboard(Request $request){
        ...
        // $commission = new TypeResource($commission);
        $commission = ProviderType::where('id', $provider->providertype_id)->first();
$commission = $commission ? new TypeResource($commission) : null;
