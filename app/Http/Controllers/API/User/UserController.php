...
class UserController extends Controller
{
    ...
    //         if (request('loginfrom') === 'vue-app' && $user->user_type != 'user') {
    //             return comman_message_response(trans('auth.not_able_login'), 400);
    //         }
    ...
    //         $success['is_verify_provider'] = (int) verify_provider_document($user->id);
    ...
    public function login(Request $request)
    {
        ...
        if (filter_var($credentials['email'], FILTER_VALIDATE_EMAIL)) {
            $field = 'email';
        } elseif (preg_match('/^\d{10}$/', $credentials['email'])) { // Assuming 10-digit mobile number
            $field = 'contact_number';
        } else {
            return response()->json(['message' => trans('auth.failed')], 406);
        }
    ...
    public function login1(Request $request)
    {
        ...
        if (filter_var($credentials['login_input'], FILTER_VALIDATE_EMAIL)) {
            $field = 'email';
        } elseif (preg_match('/^\d{10}$/', $credentials['login_input'])) { // Assuming 10-digit mobile number
            $field = 'contact_number';
        } else {
            return response()->json(['message' => trans('auth.failed')], 406);
        }

        // Attempt to log in using the detected field
        if (Auth::attempt([$field => $credentials['login_input'], 'password' => $credentials['password']])) {
            $user = Auth::user();
            ...
            // if ($request->input('loginfrom') === 'vue-app' && $user->user_type != 'user') {
            //     return response()->json(['message' => trans('auth.not_able_login')], 400);
            // }
    ...
    private function storeLoginDetails($request, $user)
    {
        $ip = $request->ip();
        $userAgent = $request->header('User-Agent');
        $today = Carbon::now()->format('Y-m-d');

        // Use Jenssegers Agent to get device info
        $agent = new Agent();
        $agent->setUserAgent($userAgent);

        $device = $agent->device(); // Device Name
        $platform = $agent->platform(); // OS (e.g., Windows, iOS, Android)
        $browser = $agent->browser(); // Browser Name
        $appName = $request->header('X-App-Name', 'Web'); // Custom Header for App Name (if applicable)

        // Check if this is the first login of the day
        $existingLog = UserLoginLog::where('user_id', $user->id)
            ->whereDate('log_datetime', $today)
            ->exists();

        if (!$existingLog) {
            // Determine if user is new or existing
            $userStatus = ($user->created_at->format('Y-m-d') == $today) ? 'new' : 'existing';

            // Insert into first table (user_login_logs)
            UserLoginLog::create([
                'user_id' => $user->id,
        ...
        // Insert into second table (user_login_activities)
        UserLoginActivity::create([
            ...
            'data' => json_encode([
                    ...
                    'platform' => $platform,
                    'browser' => $browser,
                    'app_name' => $appName, // If coming from a mobile app
                ]),
        ]);
    }
}
