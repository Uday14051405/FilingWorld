...
class PaymentController extends Controller
{
    ...
    public function savePayment(Request $request)
    {
        ...
        $is_product = '';
        $service_id1 = Booking::where('id',$request->booking_id)->pluck('service_id');
            $user_type1 = Booking::where('id',$request->booking_id)->value('type');
            $user_data1 = Product::where('id',$service_id1)->value('is_enable_advance_payment');
            
            if($user_type1 == 'product'){
                $is_product = 'product';
                if($user_data1 == 1){
            ...
            if ($existingPayment) {
                return response()->json(['message' => 'Duplicate payment detected.'], 409);
            }
        ...
        $service_id = Booking::where('id',$request->booking_id)->pluck('service_id');
            $user_type = Booking::where('id',$request->booking_id)->value('type');
            $user_data = Product::where('id',$service_id)->value('is_enable_advance_payment');
        ...
        if($is_product == 'product'){
            $service = Product::where('id',$service_id)->first();  
        }else{
