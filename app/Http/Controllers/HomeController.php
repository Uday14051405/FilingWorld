...
$data['datetime'] = $sitesetup ? json_decode($sitesetup->value) : null;
    ...
    public function changeStatus(Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'subcategory_featured':
                ...
                break;
            case 'menu_status':
                $menu = \App\Models\MenuCategory::find($request->id);
                $menu->status = $request->status;
                $menu->save();
                break;
            case 'menu_featured':
                $message_form = __('messages.menu');
                $menu = \App\Models\MenuCategory::find($request->id);
                $menu->is_featured = $request->status;
                $menu->save();
                break;
            case 'submenu_status':
                $submenu = \App\Models\SubMenu::find($request->id);
                $submenu->status = $request->status;
                $submenu->save();
                break;
            case 'submenu_featured':
                $message_form = __('messages.submenu');
                $submenu = \App\Models\SubMenu::find($request->id);
                $submenu->is_featured = $request->status;
                $submenu->save();
                break;
            case 'product_menu_status':
                $product_menu = \App\Models\ProductMenuCategory::find($request->id);
                $product_menu->status = $request->status;
                $product_menu->save();
                break;
            case 'product_menu_featured':
                $message_form = __('messages.product_menu_category');
                $product_menu = \App\Models\ProductMenuCategory::find($request->id);
                $product_menu->is_featured = $request->status;
                $product_menu->save();
                break;
            case 'product_submenu_status':
                $product_submenu = \App\Models\ProductSubMenu::find($request->id);
                $product_submenu->status = $request->status;
                $product_submenu->save();
                break;
            case 'product_submenu_featured':
                $message_form = __('messages.product_sub_menu_category');
                $product_submenu = \App\Models\ProductSubMenu::find($request->id);
                $product_submenu->is_featured = $request->status;
                $product_submenu->save();
                break;
            case 'product_category_status':
                $product_category = \App\Models\ProductCategory::find($request->id);
                $product_category->status = $request->status;
                $product_category->save();
                break;
            case 'product_category_featured':
                $message_form = __('messages.product_category');
                $product_category = \App\Models\ProductCategory::find($request->id);
                $product_category->is_featured = $request->status;
                $product_category->save();
                break;
            case 'product_subcategory_status':
                $product_subcategory = \App\Models\ProductSubCategory::find($request->id);
                $product_subcategory->status = $request->status;
                $product_subcategory->save();
                break;
            case 'product_subcategory_featured':
                $message_form = __('messages.product_subcategory');
                $product_subcategory = \App\Models\ProductSubCategory::find($request->id);
                $product_subcategory->is_featured = $request->status;
                $product_subcategory->save();
                break;
            case 'product_status':
                ...
                $product->save();
                break;
            case 'productpackage_status':
                $productpackage = \App\Models\ProductPackage::find($request->id);
                $productpackage->status = $request->status;
                $productpackage->save();
                break;
            case 'productaddon_status':
                $productaddon = \App\Models\ProductAddon::find($request->id);
                $productaddon->status = $request->status;
                $productaddon->save();
                break;
            case 'plan_status':
    ...
    public function getProductAjaxList(Request $request)
    {
        $items = array();
        $value = $request->q;
        ...
        switch ($request->type) {
            case 'permission':
                ...
                break;
            case 'category':
                $items = \App\Models\ProductCategory::select('id', 'name as text')->where('status', 1);
                if (isset($request->is_featured)) {
                    $items->where('is_featured', $request->is_featured);
                }
                if ($value != '') {
                ...
                case 'category_admin':
                    ...
                    break;
            case 'subcategory':
                $items = \App\Models\ProductSubCategory::select('id', 'name as text')->where('status', 1);
            ...
            case 'handyman':
                ...
                break;
            case 'product':
                $items = \App\Models\Product::select('id', 'name as text')->where('status', 1);
                ...
                if (isset($request->hot_selling)) {

                    // $topBookedProducts = Booking::select('service_id', DB::raw('COUNT(*) as total_bookings'))
                    //     ->where('type', 'product')
                    //     ->groupBy('service_id')
                    //     ->orderByDesc('total_bookings')
                    //     ->get();
                
                    // // Extract service_ids into an array
                    // $serviceIds = $topBookedProducts->pluck('service_id')->toArray();
                
                    // if (!empty($serviceIds)) {
                    //     $items->whereIn('id', $serviceIds)
                    //           ->orderByRaw("FIELD(id, " . implode(',', $serviceIds) . ")");
                    // } else {
                        // Optional: Handle case where no services match the criteria
                        $items->whereRaw('1 = 1'); // Ensures no results are returned
                    // }
                }
            ...
            case 'menu_list':
                $items = \App\Models\MenuCategory::select('id', 'name as text')->where('status', 1);
            ...
            case 'product_menu_list':
                $items = \App\Models\ProductMenuCategory::select('id', 'name as text')->where('status', 1);
            ...
            case 'blog_list':
                $items = \App\Models\Blog::select('id', 'title as text')->where('status', 1);
            ...
            case 'service-list':
                $items = \App\Models\Product::select('id', 'name as text','price')
                ->where('status', 1)
                ->where('service_type', 'product');
                // Apply search filter if $value is provided
                if (!empty($value)) {
                    $items->where('name', 'LIKE', '%' . $value . '%');
                }
            ...
            case 'subcategory_list':
                $category_id = !empty($request->category_id) ? $request->category_id : '';
                $items = \App\Models\ProductSubCategory::select('id', 'name as text')->where('category_id', $category_id)->where('status', 1);
                $items = $items->get();
                break;
            case 'service_package':
                $service_id = !empty($request->service_id) ? $request->service_id : $auth_user->id;
                $items = \App\Models\ProductPackage::select('id', 'description as text', 'status')->where('provider_id', $service_id)->where('status', 1);
                $items = $items->get();
                break;
            case 'all_user':
                $items = \App\Models\User::select('id', 'display_name as text')
    ...
    function getAjaxProductList(Request $request)
    {
        $items = \App\Models\Product::select('id', 'name as text','price')->where('status', 1)->where('type', 'fixed');
