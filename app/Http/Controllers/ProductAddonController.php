...
class ProductAddonController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $assets = ['datatable'];
        return view('productaddon.index', compact('pageTitle','auth_user','assets','filter'));
    }

    public function index_data(DataTables $datatable,Request $request)
    {
        $query = ProductAddon::query()->ServiceAddon()->with(['translations', 'service.translations']);
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('status' , function ($query){
                return '<div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                    <div class="custom-switch-inner">
                        <input type="checkbox" class="custom-control-input  change_status" data-type="productaddon_status" '.($query->status ? "checked" : "").'  value="'.$query->id.'" id="'.$query->id.'" data-id="'.$query->id.'">
            ...
            ->editColumn('name', function($query) use($primary_locale){
                ...
                $link = '<a class="btn-link btn-link-hover"  href='.route('productaddon.create', ['id' => $query->id]).'>'.$name.'</a>';
                return $link ?? '-';
            })
            ...
            ->addColumn('action', function ($serviceaddon) {
                return view('productaddon.action', compact('serviceaddon'))->render();
            })
            ->addIndexColumn()
    ...
    public function bulk_action(Request $request)
    {
        ...
        switch ($actionType) {
            case 'change-status':
                $branches = ProductAddon::whereIn('id', $ids)->update(['status' => $request->status]);
    ...
    public function create(Request $request)
    {
        ...
        $serviceaddon = ProductAddon::find($id);
        $pageTitle = trans('messages.update_form_title',['form'=>trans('messages.product_addon')]);
        
        if($serviceaddon == null){
            $pageTitle = trans('messages.add_button_form',['form' => trans('messages.product_addon')]);
            $serviceaddon = new ProductAddon;
        }else{
           
            if (optional($serviceaddon->service)->provider_id !== auth()->user()->id && !auth()->user()->hasRole(['admin', 'demo_admin'])) {
                return redirect(route('productaddon.index'))->withErrors(trans('messages.demo_permission_denied'));
            }
        }
        
        return view('productaddon.create', compact('pageTitle' ,'serviceaddon' ,'auth_user','language_array'));
    }
    ...
    public function store(ServiceAddonRequest $request)
    {
        ...
        $translatableAttributes = ['name'];
         ...
         if (!$request->is('api/*') && is_null($request->id) && !$request->hasFile('serviceaddon_image')) {
             return redirect()->route('productaddon.create')
        ...
        $result = ProductAddon::updateOrCreate(['id' => $data['id'] ],$data);
        if ($request->is('api/*')) {
        ...
        if ($request->hasFile('serviceaddon_image')) {
            ...
            return redirect()->route('productaddon.create', ['id' => $result->id])
            ->withErrors(['serviceaddon_image' => 'The attachments field is required.'])
            ->withInput();
        }	
        
        $message = trans('messages.update_form',['form' => trans('messages.product_addon')]);
        if($result->wasRecentlyCreated){
            $message = trans('messages.save_form',['form' => trans('messages.product_addon')]);
        }
        if($request->is('api/*')) {
            $response = [
                'message'=>$message,
        ...
        return redirect(route('productaddon.index'))->withSuccess($message); 
    }
    ...
    public function destroy($id)
    {
        ...
        $serviceaddon = ProductAddon::find($id);
        $msg= __('messages.msg_fail_to_delete',['item' => __('messages.product_addon')] );
        
        if($serviceaddon != '') { 
            $serviceaddon->delete();
            $msg= __('messages.msg_deleted',['name' => __('messages.product_addon')] );
        }
        if(request()->is('api/*')){
