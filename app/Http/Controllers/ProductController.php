...
class ProductController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->is('servicepackage/list/*')) { 
            if ($auth_user->hasRole('provider')) {
                $isAuthorizedPackage = ProductPackage::where('id', $servicepackage)
        ...
        $assets = ['datatable'];
        return view('product.index', compact('pageTitle','auth_user','assets','filter','postrequestid','servicepackage'));
    } 

    public function index_data(DataTables $datatable,Request $request)
    {
        $query = Product::query()->myService()->with(['translations', 'category.translations']);
        ...
        if (auth()->user()->hasAnyRole(['admin','provider'])) {
            $query = $query->where('service_type','product')->withTrashed();

        }
        if ($request->has('postrequestid')) {
            $postRequestId = $request->postrequestid;
            $query = Product::whereHas('postJobService', function ($query) use ($postRequestId) {
                $query->where('post_request_id', $postRequestId);
            });
        }
        if ($request->has('servicepackage')) {
            $servicepackage = $request->servicepackage;
            $query = Product::whereHas('servicePackage', function ($query) use ($servicepackage) {
        ...
        return $datatable->eloquent($query)
            ...
            ->editColumn('name', function($query) use($primary_locale){
                ...
                if (auth()->user()->can('product edit')) {
                    return '<a class="btn-link btn-link-hover" href="' . route('product.create', ['id' => $query->id]) . '">' . $name . '</a>';
                }
            ...
            ->editColumn('provider_id' , function ($query){
                return view('product.service', compact('query'));
            })
            ...
            ->addColumn('action', function ($data) {
                $users = User::select('id', 'username')
                            ...
                            ->where('status', 1)
                            ->get();
                return view('product.action', compact('data','users'));
            })
    ...
    public function bulk_action(Request $request)
    {
        ...
        switch ($actionType) {
            case 'change-status':
                $branches = Product::whereIn('id', $ids)->update(['status' => $request->status]);
                $message = 'Bulk Product Status Updated';
                break;

            case 'delete':
                Product::whereIn('id', $ids)->delete();
                $message = 'Bulk Product Deleted';
                break;

            case 'restore':
                Product::whereIn('id', $ids)->restore();
                $message = 'Bulk Product Restored';
                break;

            case 'permanently-delete':
                Product::whereIn('id', $ids)->forceDelete();
                $message = 'Bulk Product Permanently Deleted';
                break;
    ...
    public function getUserServiceList(Request $request)
    {
        ...
        $pageTitle = __('messages.list_form_title',['form' => __('messages.products')] );
        $auth_user = authSession();
        $assets = ['datatable'];
        return view('product.user_service_list', compact('pageTitle','auth_user','assets','filter'));
    ...
    public function create(Request $request)
    {
        ...
        $pageTitle = __('messages.update_form_title',['form'=> __('messages.products')]);

        if($servicedata == null){
            $pageTitle = __('messages.add_button_form',['form' => __('messages.products')]);
        ...
        return view('product.create', compact('language_array','pageTitle' ,'servicedata' ,'auth_user' , 'advancedPaymentSetting','visittype','slotservice'));
    }
    ...
    public function store(ServiceRequest $request)
    {
        ...
        $services['service_type'] = !empty($request->service_type) ? $request->service_type : 'product';
        ...
        if($request->id == null && default_earning_type() === 'subscription'){
           ...
           if(!empty($exceed)){
            if($exceed == 1){
                $message = __('messages.limit_exceed',['name'=>__('messages.product')]);
            }else{
                 $message = __('messages.not_in_plan',['name'=>__('messages.product')]);
            }
             if($request->is('api/*')){
        ...
        if(!$request->is('api/*')) {
            ...
            if($request->has('is_enable_advance_payment')){
                ...
                $services['advance_payment_amount'] = 100;
            }
        ...
        // }
        $result = Product::updateOrCreate(['id' => $request->id], $services);
        if ($request->is('api/*')) {
        ...
        if($request->is('api/*')){
            ...
            if ($request->hasFile('service_attachment')) {
                ...
                return redirect()->route('product.create', ['id' => $result->id])
                ->withErrors(['service_attachment' => 'The attachments field is required.'])
                ->withInput();
            }	
		}
        $message = __('messages.update_form',[ 'form' => __('messages.product') ] );
		if($result->wasRecentlyCreated){
			$message = __('messages.save_form',[ 'form' => __('messages.product') ] );
		}
    ...
    public function show(Request $request, $id)
    {
        ...
        return view('product.view', compact('pageTitle','providerdata', 'auth_user', 'tabpage'));
    }
    ...
    public function action(Request $request){
        $id = $request->id;
        $service = Product::withTrashed()->where('id',$id)->first();
    ...
    public function deleteFavouriteService(Request $request)
    {
        ...
        $service_rating = UserFavouriteProduct::where('user_id',$request->user_id)->where('service_id',$request->service_id)->delete();
