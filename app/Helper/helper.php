    ...
    if (getFileExistsCheck($media)) {
        ...
        switch ($collection) {
            ...
            case 'logo':
                ...
                break;
            case 'title_logo':
                $media = asset('images/title_logo.png');
                break;
            case 'favicon':
                $media = asset('images/favicon.png');
                break;
...
function getAccessToken()
{
    ...
    \Log::info('Storage directory path:', ['path' => $directory]);
    
    $credentialsFiles = File::glob($directory . '/*.json');
    \Log::info('Found credential files:', ['files' => $credentialsFiles]);
    
    if (!empty($credentialsFiles)) {
        \Log::info('Using credential file:', ['file' => $credentialsFiles[0]]);
    
        $jsonContent = json_decode(file_get_contents($credentialsFiles[0]), true);
        \Log::info('Parsed JSON content:', ['content' => $jsonContent]);
    
        if (!isset($jsonContent['client_id'])) {
            \Log::error('Missing client_id in JSON credentials file.');
            return null;
        }
        ...
        try {
            ...
            \Log::info('Fetched access token:', ['token' => $token]);
        ...
        \Log::warning('No JSON credential files found in the directory.');
    }
