    ...
    <div class=" mt-5 justify-content-center  service-slide-items-4">
        <div class="col">
            <ProductCard 
              ...
              :cod="service.is_enable_advance_payment"
              :user_type="user_type" 
              />
        </div>
    </div>
...
import ProductShimmer  from '../shimmer/ProductShimmer.vue';
// Import Swiper Vue.js components
import { Swiper, SwiperSlide } from 'swiper/vue';

const modules = [Navigation, Pagination, Scrollbar, A11y];
const props = defineProps(['user_id','favourite','type','user_type']);
...
import { PRODUCT_API,PROVIDER_API} from '../data/api'; 
...
const [productssection] = useObserveSection(() => store.get_service_list({per_page: 8, category_id: null}))

const fetchService = async () => {
      try {
         IS_LOADER.value=true
         const response = await fetch(PRODUCT_API({ per_page: 'all', status: 1 }));
  ...
  try {
    ...
    const settings = store.landing_page_setting_list_data.data.find(
      setting => 
        (props.type === 'ac' && setting.key === 'section_10') ||
        (props.type === 'cleaning' && setting.key === 'section_11') ||
