<template>
   ...
   <div class="iq-image position-relative">
      ...
      <a :href="`${baseUrl}/product-detail/${service_id}`"  @click="storeRecentlyViewed" class="service-img">
  ...
  <div class="flex-grow-1 me-2">
    <a :href="`${baseUrl}/product-detail/${service_id}`" class="service-heading d-block p-0" @click="storeRecentlyViewed">
    ...
    <ul class="list-inline p-0 mt-1 mb-0 price-content">
      ...
      <li v-if="duration && duration !== '00:00'" class="d-inline-block fw-500 position-relative service-price">
  ...
  <!-- Book Now Button -->
  <div>
   <a v-if="user_type === 'user'" :href="`${baseUrl}/book-product/?id=${service_id}`" class="btn btn-sm btn-primary">
      {{ $t('Book Now') }}
   </a>
   <a v-else :href="`${baseUrl}/login-page?product_id=${service_id}`" class="btn btn-sm btn-primary">
      {{ $t('Book Now') }}
   </a>
  </div>
</div>
...
<div class="mt-3 d-flex justify-content-between align-items-start">
  <!-- Left side: provider image and name -->
  <div>
    <div class="d-flex align-items-center gap-2">
      ...
      <a :href="`${baseUrl}/provider-detail/${provider_id}`">
        <span class="font-size-14 service-user-name">{{ userName }}</span>
      </a>
    </div>
    <div class="d-flex align-items-center gap-1 f-none mt-2">
      ...
      <h6 class="font-size-14">
        {{ reviewNo }}
        <a :href="`${baseUrl}/rating-all?service_id=${service_id}`">
          <span v-if="reviewCount > 1" class="text-body ms-1">
            ({{ reviewCount }} {{ $t('messages.reviews') }})
          </span>
          <span v-else class="text-body ms-1">
  ...
  <!-- Right side: COD Image -->
  <div v-if="!cod" class="d-flex align-items-center gap-2">
      <img :src="`${baseUrl}/images/cod.png`" alt="COD" class="img-fluid" style="height: 60px;">
   </div>
