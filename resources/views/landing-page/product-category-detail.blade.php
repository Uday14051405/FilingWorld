            ...
            <div class="row">
                <div class="col-lg-4">
                    ...
                    <ul class="nav nav-tabs align-items-start gap-5 bg-transparent pb-0 mt-5 mb-0">
                        ...
                        <li class="nav-item">
                            <a href="{{ route('product.subcategory.list', ['category_id' => $category->id]) }}" class="nav-link rounded-3">
                                <div class="category-tabs-image">
                    ...
                    <div class="tab-content">
                       ...
                       @if( $serviceData )
                        <div class="tab-pane active show" id="all">
                                <product-list-section :user_id="{{json_encode($auth_user_id)}}" :user_type={{json_encode($auth_user_type)}} :service="{{ json_encode($serviceData) }}" :favourite="{{json_encode($favourite)}}" ></product-list-section>
                        </div>
                        @endif
                        @foreach($sub_category as $subCategory)
                            @php
                            ...
                            <div class="tab-pane" id="id-{{ $subCategory->id }}">
                                <product-list-section :user_id="{{json_encode($auth_user_id)}}" :user_type={{json_encode($auth_user_type)}} :service="{{ json_encode(array_values($filteredServices)) }}" :favourite="{{json_encode($favourite)}}"></product-list-section>
                            </div>
                        @endforeach
