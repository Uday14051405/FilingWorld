    ...
    <script>
        setTimeout(function () {
            ...
            if (alertBox) {
                ...
                alertBox.classList.add('fade');    // Optional: keep fade effect
                ...
                <div class="p-5 bg-light rounded-3">
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            ...
                            @endif
                                ...
                                @if($bookingData['booking_detail']['status'] === 'pending' &&
                                    ...
                                    @if($bookingData['booking_detail']['booking_type'] === 'service')
                                    <div class="mt-4" id="upload-document-section">
                                        <div class="card card-body shadow-sm" style="padding: 20px;">
                                            <h5 class="mb-3 text-primary text-capitalize">{{ __('Upload Document') }}</h5>

                                            <form action="{{ route('upload.document') }}" method="POST" enctype="multipart/form-data">
                                                @csrf
                                                <input type="hidden" name="order_id" value="{{ $bookingData['booking_detail']['id'] }}">

                                                <div class="row g-3 align-items-center">
                                                    <div class="col-md-4 col-sm-6">
                                                        <label for="document_name" class="form-label text-capitalize">{{ __('Document Name') }}</label>
                                                        <select name="document_name" id="document_name" class="form-select" required>
                                                            <option value="" disabled selected>{{ __('Select Document') }}</option>
                                                        </select>
                                                    </div>

                                                    <div class="col-md-4 col-sm-6" id="file_name_group" style="display: none;">
                                                        <label for="file_name" class="form-label text-capitalize">{{ __('File Name') }}</label>
                                                        <input type="text" name="other_document" id="file_name" class="form-control" placeholder="Enter file name">
                                                    </div>

                                                    <div class="col-md-4 col-sm-12">
                                                        <label for="document_file" class="form-label text-capitalize">{{ __('Upload File') }}</label>
                                                        <input type="file" name="file" id="document_file" class="form-control" accept=".jpg,.jpeg,.png,.pdf,.doc,.docx,.zip,.rar" required>
                                                    </div>

                                                    <div class="col-12 mt-3">
                                                        <button type="submit" class="btn btn-outline-secondary text-capitalize">{{ __('Upload') }}</button>
                                                    </div>
                                                </div>
                                            </form>
                                        </div>
                                    ...
                                    <div class="mt-4">
                                        <button class="btn btn-sm btn-outline-primary mb-2" type="button" id="toggle-documents">
                                            {{ __('Show Uploaded Documents') }}
                                        </button>

                                        <div id="uploaded-documents" class="table-responsive" style="display: none;">
                                                ...
                                                <tbody>
                                                    ...
                                                    @forelse($uploadedDocuments as $doc)
                                                        <tr>
                                                            <td>{{ $i++ }}</td>
                                                            <td>
                                                                {{ $doc->document_name === 'Other' ? $doc->other_document : $doc->document_name }}
                                                            </td>
                                                            <td class="text-center">
                                                                <a href="{{ url('storage/documents/' . $doc->file_name) }}" target="_blank" class="btn btn-sm btn-primary">
                                                                    {{ __('View') }}
                                                                </a>
                                                            </td>
                                                        </tr>
                                                    @empty
                                                        <tr>
                                                            <td colspan="3" class="text-center text-muted">{{ __('No documents uploaded.') }}</td>
                                                        </tr>
                                                    @endforelse
                                                </tbody>
                                            </table>
                        ...
                        <div class="col-md-4 text-md-end mt-5 mt-md-0">
                            ...
                            @endphp
                            <img src="{{ $imageSource ?? asset('images/default.png') }}" width="357" height="253" class="mw-100 rounded-3 object-cover" alt="service-detail" style="object-fit: contain;">
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const docSelect = document.getElementById("document_name");
        const fileNameGroup = document.getElementById("file_name_group");
        const fileNameInput = document.getElementById("file_name");


        // Fetch document names from the API
        fetch("{{ url('/api/active-document-names') }}")
            ...
            .then(result => {
                if (result.status && result.data.length > 0) {
                    ...
                    // docSelect.appendChild(otherOption);
                }
            })
            .catch(error => {
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const toggleButton = document.getElementById("toggle-documents");
        const tableSection = document.getElementById("uploaded-documents");

        toggleButton.addEventListener("click", function () {
            const isVisible = tableSection.style.display === "block";
            tableSection.style.display = isVisible ? "none" : "block";
            toggleButton.textContent = isVisible ? "Show Uploaded Documents" : "Hide Uploaded Documents";
        });
    });
</script>
