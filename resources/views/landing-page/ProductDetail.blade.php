        ...
        <div class="row">
            <div class="col-lg-8 pe-xxl-5">
                    ...
                    <ul class="service-meta-list list-inline m-0 d-flex align-items-center flex-wrap">
                        <li>
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <h6>{{ round($serviceData['service_detail']['total_rating'],1) }}<span class="text-body"> <a href="{{route('rating.all', ['service_id' => $serviceData['service_detail']['id'], 'type' => $type])}}"> ({{ $serviceData['service_detail']['total_review'] }} {{__('messages.reviews')}})</span></a></h6>
                ...
                <div class="mt-5 pt-lg-5 pt-3">
                    <h5 class="mb-3">{{__('landingpage.about_product')}}</h5>
                    <p class="m-0">
                    ...
                    <div class="p-5 border rounded-3">
                        ...
                        <div class="mt-5 border-top">
                            ...
                            <div class="text-end">
                                ...
                                <a href="{{ route('book.product', ['id' => $serviceData['service_detail']['id']]) }}" class="btn btn-lg btn-primary continue-button">{{__('messages.continue')}} ({{ getPriceFormat($total) }})</a>
                                @else
                                <a href="{{ route('user.login', ['product_id' =>$serviceData['service_detail']['id']]) }}" class="btn btn-lg btn-primary">{{__('messages.continue')}} ({{ getPriceFormat($total) }})</a>
                                @endif
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-5 pt-lg-5 pt-3">
                    <div class="row align-item-center">
                        <div class="col-sm-6">
                            <h5 class="text-capitalize">{{__('landingpage.select_product_package')}}</h5>
                        </div>
                        @if(count($serviceData['service_detail']['servicePackage']) > 3)
                        <div class="col-sm-6 mt-sm-0 mt-3 text-sm-end">
                            <a href="{{route('product.package',['id' => $serviceData['service_detail']['id']] )}}">{{__('messages.view_all')}}</a>
                        </div>
                        @endif
                    </div>
                    <div class="mt-5">
                ...
                <div class="section-padding px-0 pb-0">
                    <div class="row align-items-center mb-5">
                        ...
                        <div class="col-sm-3 mt-sm-0 mt-3 text-sm-end">
                            <a href="{{route('rating.all', ['service_id' => $serviceData['service_detail']['id'], 'type' => $type])}}">{{__('messages.view_all')}}</a>
                        </div>
                        @endif
                    </div>
                ...
                <div class="mt-5 pt-lg-5 pt-3">
                    <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
                        <h5 class="text-capitalize">{{__('landingpage.related_products')}}</h5>
                        <a href="{{route('product.list', ['category_id' => $serviceData['service_detail']['category_id']])}}">{{__('messages.view_all')}}</a>
            ...
            <div class="col-lg-4 ps-xxl-5 mt-lg-0 mt-5">
                <div class="bg-light p-5 rounded-3">
                    ...
                    <div class="mt-4 pt-4 ">
                        @if(auth()->check() && auth()->user()->user_type == 'user')
                        <a href="{{ route('book.product', ['id' => $serviceData['service_detail']['id']]) }}" class="btn btn-primary w-100 continue-button">{{__('messages.continue')}}</a>
                        @else
                        <a href="{{ route('user.login',['product_id' =>$serviceData['service_detail']['id']]) }}" class="btn btn-primary w-100">{{__('messages.continue')}}</a>
                        @endif
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        function updateContinueButtonHref() {
            ...
            var baseUrl = `{{ route('book.product', ['id' => $serviceData['service_detail']['id']]) }}`;
