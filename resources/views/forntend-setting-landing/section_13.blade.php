...
<div class="form-group">
    <div class="form-control d-flex align-items-center justify-content-between">
                    <label for="enable_section_13" class="mb-0">{{__('messages.enable_section_13')}}</label>
        <div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                        <input type="checkbox" class="custom-control-input section_13" name="status" id="section_13" data-type="section_13"  {{!empty($landing_page) && $landing_page->status == 1 ? 'checked' : ''}}>
            <label class="custom-control-label" for="section_13"></label>
        </div>
    </div>
</div>
        <div class="form-section" id='enable_section_13'>
    <div class="form-group">
    ...
    <div class="form-group" id='enable_select_service'>
        {{ html()->label(__('messages.select_name', ['select' => __('messages.subcategory')]) . ' <span class="text-danger">*</span>', 'name')->class('form-control-label') }}
        <br />
        {{ html()->select('service_id[]', [])
            ...
            ->attribute('data-ajax--url', route('product.ajax-list', ['type' => 'service_menu', 'is_featured' => 1]))
            ->multiple()
        }}
    </div>
...
<script>
    var enable_section_13 = $("input[name='status']").prop('checked');
    checkSection3(enable_section_13);

    $('#section_13').change(function() {
    ...
    function checkSection3(value) {
        if (value == true) {
            $('#enable_section_13').removeClass('d-none');
            $('#title').prop('required', true);
            $('#service_id').prop('required', false).trigger('change.select2');
        } else {
            $('#enable_section_13').addClass('d-none');
    ...
    function getConfig(type) {
        ...
        $.ajax({
            ...
            success: function (response) {
                var obj = '';
                var section_13 = title = service_ids = '';

                if (response) {
                    if (response.data.key == 'section_13') {
    ...
    var service_route = "{{ route('product.ajax-list', ['type' => 'service_menu']) }}";
