...
<div class="form-group">
    <div class="form-control d-flex align-items-center justify-content-between">
                    <label for="enable_section_19" class="mb-0">{{__('messages.enable_section_19')}}</label>
        <div class="custom-control custom-switch custom-switch-text custom-switch-color custom-control-inline">
                        <input type="checkbox" class="custom-control-input section_19" name="status" id="section_19" data-type="section_19"  {{!empty($landing_page) && $landing_page->status == 1 ? 'checked' : ''}}>
            <label class="custom-control-label" for="section_19"></label>
        </div>
    </div>
</div>
        <div class="form-section" id='enable_section_19'>
    <div class="form-group">
    ...
    <div class="form-group" id='enable_select_service'>
        {{ html()->label(__('messages.select_name', ['select' => __('messages.blogs')]), 'name')->class('form-control-label') }}
        <br />
        {{ html()->select('service_id[]', [])
            ->value(old('service_id'))
            ->class('select2js form-control service_id')
            ->id('service_id')
            ->attribute('data-placeholder', __('messages.select_name', ['select' => __('messages.blogs')]))
            ->attribute('data-ajax--url', route('product.ajax-list', ['type' => 'blog_list']))
            ->multiple()
        }}
    </div>
...
<script>
    var enable_section_19 = $("input[name='status']").prop('checked');
    checkSection3(enable_section_19);

    $('#section_19').change(function() {
    ...
    function checkSection3(value) {
        if (value == true) {
            $('#enable_section_19').removeClass('d-none');
            $('#title').prop('required', true);
            $('#service_id').prop('required', false).trigger('change.select2');
        } else {
            $('#enable_section_19').addClass('d-none');
    ...
    function getConfig(type) {
        ...
        $.ajax({
            ...
            success: function (response) {
                var obj = '';
                var section_19 = title = service_ids = '';

                if (response) {
                    if (response.data.key == 'section_19') {
    ...
    var service_route = "{{ route('product.ajax-list', ['type' => 'blog_list']) }}";
    ...
    $.ajax({
        ...
        success: function(result) {
            $('#service_id').select2({
                width: '100%',
                placeholder: "{{ trans('messages.select_name',['select' => trans('messages.blogs')]) }}",
                data: result.results
            });
