                ...
                <div class="card card-block card-stretch">
                    <div class="card-body p-0">
                        <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                            <h5 class="fw-bold">{{ $pageTitle ?? trans('messages.tech_available_provider_engineer') }}</h5>
                        </div>
                    </div>
                </div>
                <div class="card">
                        ...
                        <div class="row justify-content-between gy-3">
                            <div class="col-md-9 col-lg-4 col-xl-3">
                                <div class="d-flex gap-3 align-items-center" style="width: max-content;">
                                        <select name="question_category_id" id="question_category_id" class="form-control select2">
                                            <option hidden value="">Select Category</option>
                                            @forelse($question_categories as $id => $name)
                                        ...
                                        {{-- <select name="question_id" id="question_id" class="form-control select2">
                                            <option hidden value="">Select Question</option>
                                            <option value="">No Questions Available</option>
                                        </select> --}}

                                        <select name="question_id" id="question_id" class="form-control select2">
                                            <option hidden value="">Select Question</option>
                                            @forelse($questions as $id => $question)
                                                <option value="{{ $id }}">{{ rtrim($question, ':') }}</option>
                                            @empty
                                                <option value="">No Categories Available</option>
                                            @endforelse
                                        </select>
                                        ...
                                        <select name="user_type" class="form-control select2" {{ ($auth_user->user_type == 'provider') ? 'hidden' : '' }}>
                                            <option hidden value="all">Select User Type</option>
                                        ...
                                        <button class="btn btn-primary" id="ability_filter">{{ __('messages.apply') }}</button>
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function () {
            $('#question_category_id').on('change', function () {
                ...
                if (categoryId) {
                    $.ajax({
                        url: "{{ route('get-questions') }}", // Route to fetch questions
                        type: "GET",
                        data: { category_id: categoryId },
                        success: function (data) {

                            $('#question_id').empty().append('<option hidden value="">Select Question</option>');

                            if (Object.keys(data).length > 0) {
                                $.each(data, function (key, value) {
                                    var cleanValue = value.replace(/:$/, '');
                                    $('#question_id').append('<option value="' + key + '">' + cleanValue + '</option>');
                                });
                            } else {
                                $('#question_id').append('<option value="">No Questions Available</option>');
                            }
                        }
                    });
                } else {
                    $('#question_id').html('<option hidden value="">Select Question</option>');
                }
            });
        });
    </script>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            window.renderedDataTable = $('#datatable').DataTable({
                ...
                ajax: {
                    "type": "GET",
                    "url": '{!! route('available.index_data', ['list_status' => $list_status]) !!}',
                    "data": function(d) {
                        d.search = {
                            value: $('.dt-search').val()
                        };
            ...
            document.getElementById('ability_filter').addEventListener('click', function() {
                let selectedCategory = document.querySelector('select[name="question_category_id"]').value;
                let selectedQuestion = document.querySelector('select[name="question_id"]').value;
                ...
                window.renderedDataTable.ajax.url(
                    `{!! route('check_ability.index_data') !!}?questionCategory=${selectedCategory}&questionId=${selectedQuestion}&user_type=${selectedUserType}`
                ).load();
            });
        });
