...
<div class="container-fluid">
    <div class="row">
            ...
            <div class="card card-block card-stretch">
                <div class="card-body p-0">
                    <div class="d-flex justify-content-between align-items-center p-3 flex-wrap gap-3">
                        ...
                        @if($auth_user->can('productpackage list'))
                        <a href="{{ route('productpackage.index') }}" class=" float-end btn btn-sm btn-primary"><i class="fa fa-angle-double-left"></i> {{ __('messages.back') }}</a>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-body">
                    {{ html()->form('POST', route('productpackage.store'))->attribute('enctype', 'multipart/form-data')->attribute('data-toggle', 'validator')->id('servicepackage')->open()}}
        ...
        <div class="row">
            ...
            @if(auth()->user()->hasAnyRole(['admin', 'demo_admin']))
                ...
                <div class="form-group col-md-4" id="provider_id_wrapper_{{ $language['id'] }}">
                    ...
                    <select name="provider_id"
                            ...
                            class="form-control select2js-provider"
                            data-select2-type="provider"
                            data-selected-id="{{ optional($servicepackage->providers)->id ?? '' }}"
                            data-language-id="{{ $language['id'] }}"
                            data-ajax--url="{{ route('product.ajax-list', ['type' => 'provider', 'language_id' => $language['id']]) }}"
            ...
            <div class="form-group col-md-4 d-none" id="select_subcategory_{{ $language['id'] }}">
                ...
                <select name="subcategory_id"
                        ...
                        class="form-control select2js-subcategory"
                        data-select2-type="subcategory"
                        data-selected-id="{{ $servicepackage->subcategory_id ?? '' }}"
                        data-language-id="{{ $language['id'] }}"
                        data-ajax--url="{{ route('product.ajax-list', ['type' => 'subcategory', 'category_id' => $servicepackage->category_id ?? '', 'language_id' => $language['id']]) }}"
             ...
             <!-- Service Selection -->
            <div class="form-group col-md-4">
                {{ html()->label(__('messages.select_name', ['select' => __('messages.product')]) . ' <span class="text-danger">*</span>', 'service_id')->class('form-control-label') }}
                <select name="service_id[]"
                        id="service_id_{{ $language['id'] }}"
                        class="form-control select2js-service service_id"
                        data-select2-type="service"
                        data-selected-id="{{ is_array($selectedServiceId) ? implode(',', $selectedServiceId) : $selectedServiceId }}"
                        data-language-id="{{ $language['id'] }}"
                        data-ajax--url="{{ route('product.ajax-list', ['type' => 'service-list','provider_id' => optional($servicepackage->providers)->id ?? '','category_id' => $servicepackage->category_id ?? '','subcategory_id' => $servicepackage->subcategory_id ?? '', 'language_id' => $language['id']]) }}"
                        data-placeholder="{{ __('messages.select_name', ['select' => __('messages.product')]) }}"
                        multiple>
                </select>
                <small class="help-block with-errors text-danger"></small>
            </div>
...
@section('bottom_script')
    <script type="text/javascript">
        ...
        //         var get_subcategory_list = "{{ route('product.ajax-list', [ 'type' => 'subcategory_list','category_id' =>'']) }}" + category_id;
