...
?>
{{ html()->form('DELETE', route('menu.destroy', $data->id))->attribute('data-submit', 'category'.$data->id)->open() }}
<div class="d-flex justify-content-end align-items-center">
    @if(!$data->trashed())
        @if($auth_user->can('menu category delete'))
        <a class="me-3 delete-category" href="{{ route('menu.destroy', $data->id) }}" data--submit="category{{$data->id}}" 
            data--ajax="true"
    ...
    @if(auth()->user()->hasAnyRole(['admin']) && $data->trashed())
        <a href="{{ route('menu.action',['id' => $data->id, 'type' => 'restore']) }}"
        ...
        <a href="{{ route('menu.action',['id' => $data->id, 'type' => 'forcedelete']) }}"
